...
const { createReadStream, existsSync, readFile } = require$$1;
...
const start = (args) => {
    ...
    const banner = () => {
        ...
        console.log(``);
        console.log(bgYellow(`🚩 GESTIONAR DATOS 🚩`));
        console.log(cyan(`Para gestionar las PROMOCIONES puedes ingresar a `), yellow(`${publicSite}:${port}/promociones/laceado`));
        console.log(cyan(`Para gestionar los SERVICIOS puedes ingresar a `), yellow(`${publicSite}:${port}/servicios/laceado`));
    };

    const datosServicios = '/resources/servicios/dataServicios.json'
    ...
    polka()
        ...
        .get('/servicios/editarServicio', (_, res) => {
            // Manejar la solicitud para la página principal
            res.setHeader('Content-Type', 'text/html');
            const fileStream = createReadStream('./resources/servicios/editarServicio.html');
            fileStream.pipe(res);
        })
        .post('/servicios/editar/', (req, res) => {
            ...
            readFile(datosServicios, 'utf-8', (err, data) => {
                ...
                if (jsonData.Laceado[i]) {
                    ...
                    res.end(`El índice ${i} está fuera de rango`);
                    return;
                }
