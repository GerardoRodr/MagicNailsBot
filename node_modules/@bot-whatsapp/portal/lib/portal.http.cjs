...
const start = (args) => {
    ...
    polka()
        ...
        .get('/servicios/editarMensaje', (_, res) => {
            // Manejar la solicitud para la página principal
            res.setHeader('Content-Type', 'text/html');
            const fileStream = createReadStream('./resources/servicios/editarMensaje.html');
            fileStream.pipe(res);
        })
        .get('/servicios/editarMensajeImg', (_, res) => {
            // Manejar la solicitud para la página principal
            res.setHeader('Content-Type', 'text/html');
            const fileStream = createReadStream('./resources/servicios/editarMensajeImg.html');
            fileStream.pipe(res);
        })
        ...
        .post('/editarMensaje', (req, res) => {
            ...
            const tipoServicio = req.body.idServicio;
            //PROCEDIMIENTO PARA MODIFICAR UN DATO
            readFile(datosServicios, (err, data) => {
                if (err) throw err;
                const obj = JSON.parse(data);
                
                obj[tipoServicio][idMensaje].mensaje = req.body.mensaje;

                require$$1.writeFile(datosServicios, JSON.stringify(obj), err => {
                    if (err) throw err;
            ...
            readFile(datosServicios, (err, data) => {
                ...
                const storage = multer.diskStorage({
                    destination: function (req, file, cb) {
                        cb(null, './resources/servicios/imgs/' + tipoServicio);
                    },
                ...
                upload.single('img')(req, res, function (err) {
                    if (err instanceof multer.MulterError) {
                        ...
                        console.log(`Se resubió la imagen ${req.file.filename} en la ruta: /resources/servicios/imgs/${tipoServicio}`);
                        let msg2 = `/resources/servicios/imgs/${tipoServicio}/${req.file.filename}`
        ...
        .post('/eliminarMensaje', (req, res) => {
            ...
            readFile(datosServicios, (err, data) => {
                ...
                require$$1.writeFile(datosServicios, JSON.stringify(newData), err => {
                    if (err) throw err;
        ...
        .post('/eliminarMensajesTodo', (req, res) => {
            ...
            readFile(datosServicios, (err, data) => {
                ...
                //ELIMINACION DEL MENSAJE EN EL JSON, PERO SE QUEDA UN null EN EL CAMPO
                for (let i = 0; i < obj.mechas.length; i++) {
        ...
        .post('/subirImagenServicios', (req, res) => {
            const tipoServicio = req.query.tipoServicio
            const storage = multer.diskStorage({
                destination: function (req, file, cb) {
                    cb(null, './resources/servicios/imgs/' + tipoServicio);
            ...
            upload.single('img')(req, res, function (err) {
                if (err instanceof multer.MulterError) {
                    ...
                    // La imagen se subió correctamente
                    console.log(`Se subió la imagen ${req.file.filename} en la ruta: /resources/servicios/imgs/${tipoServicio}`);
                    let msg = `/resources/servicios/imgs/${tipoServicio}/${req.file.filename}`
                    // Guardado de datos
                    let data = readFileSync(datosServicios)
                    let obj = JSON.parse(data)
                    obj[tipoServicio].push({ mensaje: msg })
